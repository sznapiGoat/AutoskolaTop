...
import { CalendarCheck, Coins, Info, Receipt } from "@phosphor-icons/react/dist/ssr";
...
export default function PricingPage() {
  const extras = services.filter((s) => s.slug !== "ridicsky-prukaz-b" && s.slug !== "jizdy-do-prahy");
  return (
    <>
      <PageHero
      ...
      <section className="container-page pb-16" aria-labelledby="kurzy-b">
        ...
        <h2 id="kurzy-b" className="sr-only">
          Varianty kurzu řidičského průkazu skupiny B
        </h2>
        <PlanCards />
      </section>

      <section className="container-page py-16" aria-labelledby="platba">
        <h2 id="platba" className="font-display text-3xl font-bold md:text-4xl">
          Jak se platí
        </h2>
        <RevealGroup className="mt-8 grid gap-4 md:grid-cols-3">
          {[
            ...
            <RevealItem key={title} className="rounded-[var(--radius-card)] bg-surface-2 p-7">
              <Icon size={30} weight="duotone" className="text-accent-text" aria-hidden="true" />
      ...
      <section className="container-page grid gap-12 py-16 lg:grid-cols-2" aria-labelledby="dalsi">
        <div>
          <h2 id="dalsi" className="font-display text-3xl font-bold md:text-4xl">
            Další kurzy
          </h2>
          <ul className="mt-8 grid gap-3">
            {extras.map((s) => (
              <li key={s.slug}>
                <Link
                  href={`/sluzby/${s.slug}`}
                  className="flex items-center justify-between gap-6 rounded-2xl border border-line bg-surface p-5 transition-colors hover:border-ink"
                >
                  <span>
                    <span className="block font-semibold">{s.name}</span>
                    <span className="text-sm text-muted">{s.priceNote}</span>
                  </span>
                  <span className="shrink-0 font-display text-xl font-bold tabular-nums">{formatPrice(s.price)}</span>
                </Link>
              </li>
            ))}
          </ul>
        ...
        <div>
          <h2 className="font-display text-3xl font-bold md:text-4xl">Příplatky a poplatky</h2>
          <ul className="mt-8 grid gap-3">
            {fees.map((f) => (
              <li key={f.label} className="flex items-center justify-between gap-6 rounded-2xl bg-surface-2 p-5">
                <span>
                  <span className="block">{f.label}</span>
                  {f.note && <span className="text-sm text-muted">{f.note}</span>}
                </span>
                <span className="shrink-0 font-semibold tabular-nums">{formatPrice(f.price)}</span>
              </li>
            ))}
          </ul>
      ...
      <section className="container-page py-16" aria-labelledby="faq-platba">
        <h2 id="faq-platba" className="font-display text-3xl font-bold md:text-4xl">
          Dotazy k platbě
        </h2>
        <Accordion items={paymentFaqs} className="mt-8 max-w-3xl" />

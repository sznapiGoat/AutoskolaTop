...
**Stack:** Next.js 16 (App Router, all routes static) · React 19 · TypeScript · Tailwind CSS v4 · Motion (`motion/react`) · anime.js v4 · own icon set (Phosphor only for Facebook/Instagram/WhatsApp marks)
...
| `src/components/icons/` | Own icon set drawn from the logo geometry (2.5 strokes, sharp corners, 37° wing-tip cuts, wing stripes, one orange accent). Generated by `python scripts/icons/gen.py`. `IconTile` = container with the 37° cut corner. |
